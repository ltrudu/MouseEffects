...
public sealed class FireworkEffect : EffectBase
{
    ...
    private const int MaxFireworks = 50;
    ...
    private readonly ParticleGPU[] _gpuParticles = new ParticleGPU[MaxParticles];
    private int _nextParticle;
    private int _activeParticleCount;

    private readonly FireworkRocket[] _rockets = new FireworkRocket[MaxFireworks];
    ...
    private bool _enableSparkle = true;
    private float _sparkleIntensity = 0.5f;
    private float _sparkleFrequency = 10f;
    ...
    protected override void OnConfigurationChanged()
    {
        if (Configuration.TryGet("maxFireworks", out int maxFw))
            _maxFireworks = Math.Clamp(maxFw, 1, 50);
        ...
        if (Configuration.TryGet("clickParticleCount", out int clickCount))
            _clickParticleCount = clickCount;
        ...
        if (Configuration.TryGet("moveParticleCount", out int moveCount))
            _moveParticleCount = moveCount;
        ...
        if (Configuration.TryGet("enableSparkle", out bool sparkle))
            _enableSparkle = sparkle;

        if (Configuration.TryGet("sparkleIntensity", out float sparkleInt))
            _sparkleIntensity = sparkleInt;

        if (Configuration.TryGet("sparkleFrequency", out float sparkleFreq))
            _sparkleFrequency = sparkleFreq;
    ...
    protected override void OnUpdate(GameTime gameTime, MouseState mouseState)
    {
        ...
        if (_spawnOnLeftClick && leftPressed && !_wasLeftPressed)
            SpawnFirework(mouseState.Position, _clickParticleCount, _clickExplosionForce, totalTime);
        ...
        if (_spawnOnMove)
        {
            ...
            {
                SpawnFirework(mouseState.Position, _moveParticleCount, _moveExplosionForce, totalTime);
    ...
    private void UpdateRockets(float dt, float totalTime)
    {
        for (int i = 0; i < MaxFireworks; i++)
        {
            ...
            {
                SpawnExplosion(rocket.Position, _clickParticleCount, _clickExplosionForce, rocket.Color, totalTime, isSecondary: false);
    ...
    private void UpdateParticles(float dt, float totalTime)
    {
        ...
        for (int i = 0; i < MaxParticles; i++)
        {
            ...
            if (_enableSparkle)
                p.SparklePhase += _sparkleFrequency * dt;
    ...
    private void SpawnFirework(Vector2 position, int particleCount, float force, float totalTime)
    {
        ...
        Vector4 color = GetFireworkColor(totalTime);
    ...
    private void SpawnRocket(Vector2 position, float totalTime)
    {
        for (int i = 0; i < MaxFireworks; i++)
        {
            if (!_rockets[i].IsActive)
            {
                ...
                rocket.Color = GetFireworkColor(totalTime);
                rocket.Age = 0f;
    ...
    private void SpawnRocketTrail(Vector2 position, Vector4 color)
    {
        for (int i = 0; i < 3; i++)
        {
            SpawnParticle(
                position,
                new Vector2((Random.Shared.NextSingle() - 0.5f) * 30f, Random.Shared.NextSingle() * 50f + 20f),
                color * 0.6f,
                0.3f,
                _minParticleSize * 0.5f,
                canExplode: false);
        }
    }
    ...
    private void SpawnParticle(Vector2 position, Vector2 velocity, Vector4 color, float lifespan, float size, bool canExplode)
    {
        ...
        do
        {
            ...
            if (p.Life <= 0f)
            {
                ...
                p.MaxLife = lifespan;
                p.Size = size;
                p.SparklePhase = Random.Shared.NextSingle() * MathF.PI * 2f;
    ...
    private Vector4 GetFireworkColor(float totalTime)
    {
        if (_rainbowMode)
    ...
    protected override void OnRender(IRenderContext context)
    {
        ...
        var frameData = new FrameData
        {
            ...
            EnableSparkle = _enableSparkle ? 1f : 0f,
        ...
        for (int j = activeIndex; j < MaxParticles; j++)
        {
